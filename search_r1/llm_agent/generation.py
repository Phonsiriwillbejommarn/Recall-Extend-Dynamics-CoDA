...
class GenerationConfig:
    ...
    inner_system_prompt: str = """You are a **Strict Logic Critic**.
...
Step 2: [Correct/Incorrect] - Reason...
Overall: [Pass/Fail/Needs Refinement]
"""
    inner_system_prompt_instruct: str = """You are a **Strict Logic Critic**.
Your job is to verify the reasoning provided in the <task>.
Output your evaluation clearly.
"""

    # --- Maximum Procedural Thinking Prompt (Two-Role Reasoning) ---
    max_procedural_system_prompt: str = """You are an advanced reasoning model that simulates a "Two-Role System" to solve complex problems.
You must alternate between two personas: **Actor (Generator)** and **Critic (Evaluator)**.

**Structure (You MUST follow this exactly):**
...
[Critic - Evaluation]
Step 1 Feedback: (Check for logic errors, missing info, or assumptions)
Step 2 Feedback: ...
Overall Assessment: (Pass/Fail/Needs Refinement)
...
**Constraint:**
- The Critic MUST be strict and skeptical.
- The Actor MUST use the Critic's feedback to improve the final answer.
- You are a Base Model, so strict adherence to this format is required.
"""
    enable_max_procedural: bool = False
...
If I want to give the final answer, I should put the answer between <answer> and </answer>. Let me try again.\n'
    ...
    def batch_execute(self, queries: List[str]) -> Tuple[DataProto, List[str]]:
        ...
        if self.config.enable_max_procedural:
            system_prompt_to_use = self.config.max_procedural_system_prompt
        else:
            system_prompt_to_use = self.config.inner_system_prompt

        for query in queries:
            if self.model_type == "base":
                inner_prompt = system_prompt_to_use + query + "\n" + "Assistant: "
            else:
                chat_list = [
                    {"role": "system", "content": self.config.inner_system_prompt_instruct if not self.config.enable_max_procedural else self.config.max_procedural_system_prompt},
