...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if self._is_actor:
            fsdp_config = getattr(self.config.actor, 'fsdp_config', {})
    ...
    def _build_model_optimizer(self,
        ...
        # --- LoRA Integration ---
        lora_config_dict = self.config.actor.get('lora', None)
        if self._is_actor and lora_config_dict is not None:
             if self.rank == 0:
                print(f"Loading LoRA config: {lora_config_dict}")
             
             # Convert config dict to LoraConfig object
        ...
        actor_module_fsdp = FSDP(
            ...
            use_orig_params=True if (self._is_actor and lora_config_dict is not None) else False, # LoRA requires use_orig_params=True for FSDP
...
class RewardModelWorker(Worker):
    ...
    def _forward_micro_batch(self, micro_batch):
        from verl.utils.padding_utils import pad_input, unpad_input, index_first_axis, rearrange
