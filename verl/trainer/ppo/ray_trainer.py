...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == 'gae':
        ...
        types = data.non_tensor_batch['type']
        responses = data.batch['responses']
        response_length = responses.size(-1)
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
...
def compute_generation_statistics(batch):
    """Computes statistics about the generation process for planner and executor."""
    metrics = {}
    planner_mask = np.array(batch.non_tensor_batch['type']) == 'planner'
    executor_mask = np.array(batch.non_tensor_batch['type']) == 'executor'

    # Process metrics for both roles
    for role_name, mask in [('planner', planner_mask), ('executor', executor_mask)]:
        if not np.any(mask):
            continue
        
        sub_batch = batch[mask]
        
        # Keys available for both planner and executor in non_tensor_batch
        for key in ['prompt_tokens', 'response_tokens', 'avg_gen_speed', 'avg_context_length', 'sample_time_s']:
            if key in sub_batch.non_tensor_batch:
                values = np.array(sub_batch.non_tensor_batch[key])
                # For sample_time_s, we only care about planner, executor values are 0
                if key == 'sample_time_s' and role_name == 'executor':
                    continue
                if values.size > 0:
                    metric_key = 'avg_gen_speed_tok_s' if key == 'avg_gen_speed' else key
                    metrics[f'{role_name}/{metric_key}/mean'] = values.mean()
                    metrics[f'{role_name}/{metric_key}/std'] = values.std()

        
        # Keys only for planner
        if role_name == 'planner' and 'executor_calls_count' in sub_batch.non_tensor_batch:
            values = np.array(sub_batch.non_tensor_batch['executor_calls_count'])
            if values.size > 0:
                metrics['planner/executor_calls_per_traj/mean'] = values.mean()
                metrics['planner/executor_calls_per_traj/max'] = values.max()
                metrics['planner/executor_calls_per_traj/min'] = values.min()

        # Keys only for executor
        if role_name == 'executor' and 'executor_loop_count' in sub_batch.non_tensor_batch:
            values = np.array(sub_batch.non_tensor_batch['executor_loop_count'])
            if values.size > 0:
                metrics['executor/avg_loops/mean'] = values.mean()
                metrics['executor/avg_loops/max'] = values.max()
                metrics['executor/avg_loops/min'] = values.min()
...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        # Note: Ensure batch size is compatible with distributed training (drop_last=True helps)
        ...
        logging.info(f"Filtered training dataset size: {len(self.train_dataset.dataframe)}")
        ...
        logging.info(f"Filtered validation dataset size: {len(self.val_dataset.dataframe)}")
    ...
    def _validate(self):
        ...
        all_gen_stats = defaultdict(list) 
        ...
        if not self.config.do_search:
            ...
            for batch_dict in self.val_dataloader:
                ...
                with _timer('step', timing_raw):
                    first_input_ids = test_batch.batch['input_ids'][:, -gen_config.max_start_length:].clone()
                    with _timer('gen', timing_raw):
                        generation_manager.timing_raw = timing_raw
                        final_gen_batch_output = generation_manager.run_llm_loop(
                            gen_batch=test_batch,
                            initial_input_ids=first_input_ids,
                        )
                    
                    planner_batch_size = len(test_batch.batch)
                    planner_mask = np.array([True] * planner_batch_size + [False] * (len(final_gen_batch_output) - planner_batch_size))
                    test_batch = final_gen_batch_output

                    test_batch.non_tensor_batch['type'] = np.array(['planner'] * planner_batch_size + ['executor'] * (len(test_batch.batch) - planner_batch_size))
                    
                    for key in test_batch.batch.keys():
                        test_batch.batch[key] = test_batch.batch[key].long()

                    gen_stats = compute_generation_statistics(test_batch)
                    for key, value in gen_stats.items():
                        all_gen_stats[key].append(value)
                    
                    # evaluate using reward_function
                    # for certain reward function (e.g. sandbox), the generation can overlap with reward
                    eval_scores = self.val_reward_fn.get_logging_scores(test_batch, step=self.global_steps)
                    for key, value in eval_scores.items():
                        all_eval_scores[key].append(value)

                    data_source_lst.append(test_batch[planner_mask].non_tensor_batch.get('data_source', ['unknown'] * planner_batch_size))
        ...
        for key, values in all_gen_stats.items():
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get('val_before_train', True):
            ...
            for metric_name in all_print_val_metrics:
                logging.info(f'Eval {metric_name}:')
                for key in ['nq', 'triviaqa', 'popqa', 'hotpotqa', '2wikimultihopqa', 'musique', 'bamboogle', 'mean']:
                    ...
                    logging.info(f'{val_key}: {val}')

            logging.info("Eval Generation Statistics:")
            sorted_keys = sorted(val_metrics.keys())
            for key in sorted_keys:
                if key.startswith('val/planner/') or key.startswith('val/executor/'):
                    logging.info(f'{key}: {val_metrics.get(key)}')
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # add uid for each sample
                batch.non_tensor_batch['uid'] = np.array([str(uuid.uuid4()) for _ in range(len(batch.batch))], dtype=object)
                
                planner_batch_size = len(batch.batch)
                ...
                # with _timer('step', timing_raw):
                        ...
                        first_input_ids = batch.batch['input_ids'][:, -gen_config.max_start_length:].clone().long()
                    ...
                    batch.non_tensor_batch['type'] = np.array(['planner'] * planner_batch_size + ['executor'] * (len(batch.batch) - planner_batch_size))
                ...
                # collect metrics
                planner_mask = np.array(batch.non_tensor_batch['type']) == 'planner'
                metrics.update(compute_data_metrics(batch=batch[planner_mask], use_critic=self.use_critic))
                metrics.update(compute_generation_statistics(batch))
                metrics.update(compute_timing_metrics(batch=batch[planner_mask], timing_raw=timing_raw))
