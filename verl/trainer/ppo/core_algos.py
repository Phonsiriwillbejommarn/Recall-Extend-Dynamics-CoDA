...
def compute_grpo_outcome_advantage(token_level_rewards: torch.Tensor,
    ...
    # print(f"scores_sample: {scores[:10]}, {scores[512:512+10]}")
    # print(f"index_sample: {index[:10]}, {index[512:512+10]}")
    ...
    with torch.no_grad():
        ...
        # print(f"id2std: {id2std}")
        # print(f"norm_scores_sample: {scores[:10]}, {scores[512:512+10]}")
...
def compute_red_weight(h_rl_prev, h_rl_curr, h_sft_prev, h_sft_curr, G):
    ...
    """
    # Use absolute change as per paper: delta_h = |h_curr - h_prev|
    delta_h_rl = torch.abs(h_rl_curr - h_rl_prev)
    delta_h_sft = torch.abs(h_sft_curr - h_sft_prev)
