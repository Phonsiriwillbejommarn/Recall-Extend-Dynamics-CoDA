...
class RewardManager():
    ...
    def get_logging_scores(self, data: DataProto, step: int = -1):
        planner_batch_size = (data.non_tensor_batch['type'] == 'planner').sum()

        additional_scores = defaultdict(lambda: torch.zeros(planner_batch_size, dtype=torch.float32))
        already_print_data_sources = {}

        for i in range(len(data)):
            data_item = data[i]  # DataProtoItem
            traj_type = data_item.non_tensor_batch['type']
            ...
            if traj_type == "planner":
                ...
                for j in range(len(data)):
                    if data[j].non_tensor_batch['uid'] == uid and data[j].non_tensor_batch['type'] == "executor":
                ...
                if already_print_data_sources[data_source] < self.num_examine:
                    ...
                    if self.log_path is not None:
                        ...
                        log_info = {
                            ...
                            'planner_response': sequences_str,
                            'executor_response': "\n\n".join(executor_str_list)
                        }
    ...
    def __call__(self, data: DataProto):
        ...
        for i in range(len(data)):
            ...
            if traj_type == "planner":
                ...
                if do_print:
                    answer = qa_em.extract_solution(responses_str)

                    print(f"--------------Begin Case--------------")
                    print(f"***** Begin planner *****")
                    print(f"Golden answers: {ground_truth['target']}")
                    print(f"Extracted answer: {answer}")
                    print(f"Planner solution string: {sequences_str}")
                    print(f"***** End planner *****")
                    print(f"***** Begin executor *****")

                    for j in range(len(data)):
                        if data[j].non_tensor_batch['uid'] == uid and data[j].non_tensor_batch['type'] == "executor":
                            executor_sequences_str, _ = self.decode_sequences_and_responses(data[j])
                            print(f"Executor solution string: {executor_sequences_str}")
                ...
                score = qa_em.compute_reward(responses_str=responses_str, ground_truth=ground_truth, score_func=compute_score_fn)
                score_record[uid]["planner_format_score"].append(format_validity)
                score_record[uid]["answer_score"] = score
            else:
        ...
        for i in range(len(data)):
            data_item = data[i]  # DataProtoItem
            uid = data_item.non_tensor_batch['uid']
            reward = score_record[uid]["reward"]
